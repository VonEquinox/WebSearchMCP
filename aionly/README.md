#  WebSearch MCP Server (AI Only)

一个基于 MCP (Model Context Protocol) 的智能网页搜索服务器，提供 **AI 深度搜索（仅 AI）** 与网页抓取能力。

> **推荐**: 本目录使用 `SOTASearch.py`，`web_search` 仅走 AI 深度搜索（不包含 Brave 搜索）。适合与 [CherryStudio](https://github.com/kangfenmao/cherry-studio) 配合使用。

## 功能

| 工具 | 说明 |
|------|------|
| `web_search` | AI 深度搜索（仅 AI），返回链接列表和 AI 总结 |
